...
async def claim(request):
    ...
    guild_id, user_id, period = body["guild_id"], body["user_id"], body["period"]
    ...
    new_balance = await db.adjust_balance(guild_id, user_id, amount, period)
    await db.set_cooldown(guild_id, user_id, column)
...
async def give(request):
    body = await request.json()
    guild_id, giver_id, receiver_id, amount = body["guild_id"], body["giver_id"], body["receiver_id"], body["amount"]

    await db.ensure_user(giver_id)
    await db.ensure_user(receiver_id)
    try:
        await db.adjust_balance(guild_id, giver_id, -amount, "give")
    except InsufficientBalance:
        return web.json_response({"error": "insufficient_balance"}, status=409)
    await db.adjust_balance(guild_id, receiver_id, amount, "give")
...
async def leaderboard(request):
    ...
    rows = await db.leaderboard(guild_id, limit)
...
async def admin_adjust(request):
    body = await request.json()
    guild_id, user_id, amount, reason = body["guild_id"], body["user_id"], body["amount"], body["reason"]
    await db.ensure_user(user_id)
    new_balance = await db.adjust_balance(guild_id, user_id, amount, "admin_adjust", allow_negative=True)
    logger.info("Admin-adjusted %s by %d in guild %s (%s). New balance: %d", user_id, amount, guild_id, reason, new_balance)
