...
-- Keyed by guild_id for cleanliness (not because this bot supports running
-- independently across multiple servers -- it still assumes one guild).
...
-- Inhouses: queue (ephemeral but persisted in case of restart)
