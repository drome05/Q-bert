...
class Twitch(commands.Cog):
    twitch_group = app_commands.Group(name="twitch", description="Twitch live announcement commands")
    ...
    async def link(self, interaction: discord.Interaction, username: str):
        await twitch_client.post("/link", {"user_id": str(interaction.user.id), "twitch_username": username})
    ...
    async def unlink(self, interaction: discord.Interaction):
        try:
            await twitch_client.delete(f"/unlink/{interaction.user.id}")
        except ServiceError:
    ...
    async def list_accounts(self, interaction: discord.Interaction):
        rows = await twitch_client.get("/accounts")
        if not rows:
    ...
    async def unlink_user(self, interaction: discord.Interaction, member: discord.Member):
        try:
            await twitch_client.delete(f"/unlink/{member.id}")
        except ServiceError:
    ...
    async def announce(self, interaction: discord.Interaction, member: discord.Member):
        ...
        result = await twitch_client.get(f"/live-status/{member.id}")
    ...
    async def live_poll(self):
        try:
            result = await twitch_client.post("/live-check", {})
        except ServiceError as e:
            if e.body.get("error") != "not_configured":
                logger.warning("Twitch live-poll call failed: %s", e)
            return
